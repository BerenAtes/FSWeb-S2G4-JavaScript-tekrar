...
function CemberinAlani(cemberYaricap, pi) {
  let cemberAlan = pi * Math.pow(cemberYaricap, 2);
  return cemberAlan;
}
...
ucebolunenlerintoplami = ucetambolunenler.reduce((xx, yy) => xx + yy, 0);
...
for (let tekrarEdenEleman of sayilar) {
  if (dizi[tekrarEdenEleman] !== undefined) {
    dizi[tekrarEdenEleman]++;
  } else {
    dizi[tekrarEdenEleman] = 1;
  }
}
for (let ikinciDöngü in dizi) {
  if (dizi[ikinciDöngü] > 1) {
    tekraredensayilar.push(
      `${ikinciDöngü} sayısı ${dizi[ikinciDöngü]} kere tekrar edilmiştir`
    );
  }
}
